...
> (Rn) requirement · (Pn) property. Extends existing `AnalysisTemplate`. Pairs with feature 06 (`visible_columns`) and feature 10 (verified return). **After 06.**

- [ ] 1. Model & migration
  - [ ] 1.1 Extend `AnalysisTemplate` — add `config` JSON, `is_public`, `star_count` (indexed), `profile_risk_tag` (nullable; set on defaults) (R1, R3, R5)
...
- [ ] 2. Default seeding (quality-first)
  - [ ] 2.1 Write a short thesis per `risk_tag` (who it's for, axes, columns, what should rank above what)
  - [ ] 2.2 Create `server/app/services/template_seed.py` with `DEFAULT_TEMPLATES` + `default_for_risk_tag` + idempotent `seed_default_templates()` (R3, P1)
  - [ ] 2.3 Point `suggest_default_template` at the seed so there is one source of truth (R7)
  - [ ] 2.4 Fixture ranking tests per default (P16) — Conservative vs Aggressive must disagree on a known pair
  - [ ] 2.5 Call seed at startup alongside `seed_filters()` (R3)
...
- [ ] 4. Client: types & hook
  ...
  - [ ] 4.3 First-run policy: no owned templates → apply profile default; else last-applied owned (R7, P15)
  - [ ] 4.4 Add `TEMPLATE_ENDPOINTS` to `constants/api.ts`

- [ ] 5. Client: components
  - [ ] 5.1 Create `components/common/TemplateSelector.tsx` — Starter/My/Community sections, sort control, star badges; highlight the profile match in Starter (R2, R5, R7)
  - [ ] 5.2 Create `components/common/SaveTemplateModal.tsx` (R1)
  - [ ] 5.3 Create `components/common/StarButton.tsx` (R5)
  - [ ] 5.4 Wire apply → `setActiveTemplate` + feature 06 `setVisibleColumns`; place selector in `ControlPanel` (R2, P3)
...
- [ ] 6. Tests
  ...
  - [ ] 6.2 Backend ranking fixtures per `risk_tag` (P16)
  - [ ] 6.3 Frontend `useTemplateManager.test.tsx` — apply swaps both states non-destructively (P2, P3), first-run vs owned-set policy (P15), optimistic star toggle, clone independence (P6)
