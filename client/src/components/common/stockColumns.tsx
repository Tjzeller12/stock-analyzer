...
export const radarColumn = (
  ...
  comparator: (valueA, valueB, nodeA, nodeB) => {
    const stockA = nodeA?.data;
    const stockB = nodeB?.data;

    if (!stockA && !stockB) return 0;
    if (!stockA) return -1;
    if (!stockB) return 1;

    const aScore = healthScore(radarScores[stockA.symbol] || {}, activeAxes);
    const bScore = healthScore(radarScores[stockB.symbol] || {}, activeAxes);
  ...
  cellRenderer: (params: CustomCellRendererProps<Stock>) => {
    ...
    const labels = (
      activeAxes.length ? activeAxes : Object.keys(scores)
    ).filter((k) => k in scores);
    const dataPts = labels.map((l) => scores[l]);
    const { borderColor, bgColor } = healthColor(healthScore(scores, labels));
...
export const positionColumns = (): ColDef<Stock>[] => [
  ...
  },
  {
    ...
    valueFormatter: (params: ValueFormatterParams<Stock, number>) =>
      params.value != null
        ? `${params.value >= 0 ? "+" : "-"}$${Math.abs(params.value).toLocaleString(undefined, { maximumFractionDigits: 2 })}`
        : "N/A",
  },
  {
...
const metricFromSpec = (spec: ColumnSpec): ColDef<Stock> => ({
  ...
  valueFormatter: (params: ValueFormatterParams<Stock>) =>
    formatBySpec(spec, params.value as number | string | undefined),
...
export const metricColumns = (): ColDef<Stock>[] =>
  buildMetricColumns(COLUMN_REGISTRY.map((c) => c.id));
